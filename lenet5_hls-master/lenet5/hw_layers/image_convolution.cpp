...
// Tanh는 수학 연산(exp)이 복잡해 하드웨어 구현이 까다로우며 (LUT 필요), 
// ReLU는 부호 비트(0보다 작은지)만 검사하면 되는 아주 단순한 로직입니다.
// ======================================================================
float _tanh(float x){
#pragma HLS INLINE
...
// ======================================================================
// CONVOLUTION_LAYER_1 (첫 번째 합성곱 계층)
// 32x32 입력 이미지를 5x5 필터 6개로 합성곱 연산하여 6채널 28x28 특징 맵을 만듭니다.
// HLS 최적화를 위해 BRAM(내부 메모리) 배열을 선언하고 분할(Partitioning)합니다.
// ======================================================================
...
#pragma HLS array_partition variable=OBRAM complete dim=2
	// [1] 입력 데이터 적재 (Input Loading)
	// 외부 메모리(input_feature)에서 FPGA 내부 메모리 버퍼(IBRAM)로 픽셀들을 복사해옵니다.
	// PIPELINE II=1 은 1클럭마다 1개의 데이터를 파이프라인으로 읽어오라는 최적화 명령어입니다.
	copy_input_1:
	...
	// [2] 가중치 및 바이어스 적재 (Weight & Bias Loading)
	// 첫 번째 반복(init==1)일 때만 외부 메모리에서 가중치(weights)를 내부 BRAM(WBRAM)으로 복사합니다.
	if(init){
		copy_kernel_1:
	...
	// ======================================================================
	// [3] 핵심 합성곱 연산 (Core Convolution MAC)
	// 5중첩 for문 (Batch -> Row_k -> Col_k -> Row -> Col)으로 구성됩니다.
	// 하드웨어 성능을 극대화하기 위해 메모리 접근 패턴을 최적화한 형태입니다.
	// ======================================================================
	BATCH :
	for(int batch_cnt=0; batch_cnt<image_Batch; batch_cnt++) {
		ROW_K:
			...
			for(int col_k=0;col_k<5;col_k++){
				...
				for (int row = 0; row < CONV_1_OUTPUT_WH; row++) {
					...
					#pragma HLS PIPELINE II=1
						...
						// 출력 채널 6개에 대해 곱셈기(Multiplier)를 6개 복제하여 동시에 연산합니다 (unroll).
						// 이 아이디어가 우리가 설계할 "1클럭 동시 4픽셀 MAC 연산기"의 기본 원리가 됩니다.
						D_OUT:
	...
	// [4] 결과 출력 및 활성화 함수 적용 (Output & Activation)
	// 누적이 끝난 OBRAM의 결과값에 바이어스(bias)를 더하고, 활성화 함수(_tanh)를 씌워 외부로 보냅니다.
	// 우리가 작성할 코드에서는 이 부분을 Tanh 대신 부호 비트 검사(ReLU) 로직 한 줄로 대체하게 됩니다.
	copy_output:
...
// ======================================================================
// CONVOLUTION_LAYER_2 (두 번째 합성곱 계층)
// 14x14 크기의 특징 맵(6채널)을 받아 16채널 10x10 특징 맵을 생성합니다.
// Layer 1과 원리는 똑같지만, 연산량을 줄이기 위해 특정 채널끼리만 부분적으로 곱하는 'Connection Table(tbl)' 로직이 추가되어 있습니다.
// ======================================================================
