...
def main():
    ...
    subprocess.run([
        ...
        '-O', 'binary',
        '-j', '.text',
        os.path.join(test_dir, 'lenet5.elf'),
    ...
    # 4. Convert ram.bin to dmem.hex (RAM: 16384 words = 64KB)
    ...
    with open(os.path.join(test_dir, 'dmem.hex'), 'w') as f:
        for w in ram_words:
            f.write(f"{w:08X}\n")
        # Pad the rest with 0 to fill the 64KB RAM (16384 words)
        for _ in range(16384 - len(ram_words)):
            f.write("00000000\n")
            
    print(f"Extraction successful!")
    print(f"imem.hex size: {len(rom_words)} / 4096 words")
    print(f"dmem.hex size: {len(ram_words)} / 16384 words")
