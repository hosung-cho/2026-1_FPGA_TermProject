...
set build_dir  "C:/rv32i_lcd_bd_impl"
set artifact_dir [file join $script_dir "build_ps_lcd_bd_impl"]
set report_dir [file join $script_dir "reports_ps_lcd_bd_impl"]
...
create_project -force rv32i_lenet_ps_lcd_bd_impl $build_dir -part $part_name
...
add_files -norecurse [list \
  [file join $script_dir "imem.hex"] \
  [file join $script_dir "lenet_digit7_dmem.mem"] \
]
set_property FILE_TYPE {Memory Initialization Files} [get_files [file join $script_dir "imem.hex"]]
set_property FILE_TYPE {Memory Initialization Files} [get_files [file join $script_dir "lenet_digit7_dmem.mem"]]
...
connect_bd_intf_net [get_bd_intf_pins axi_interconnect_0/M00_AXI] [get_bd_intf_pins rv32i_axi_0/S_AXI]
connect_bd_intf_net [get_bd_intf_pins axi_interconnect_0/M01_AXI] [get_bd_intf_pins tftlcd_axi_0/S_AXI]
...
if {[llength $rv32i_seg] == 0 || [llength $lcd_seg] == 0} {
...
set actual_rv32i_baseaddr [get_property OFFSET $rv32i_seg]
set actual_lcd_baseaddr [get_property OFFSET $lcd_seg]
if {$actual_rv32i_baseaddr ne $rv32i_axi_baseaddr} {
  puts "ERROR: RV32I AXI base address mismatch. expected=$rv32i_axi_baseaddr actual=$actual_rv32i_baseaddr"
  exit 1
}
if {$actual_lcd_baseaddr ne $lcd_axi_baseaddr} {
  puts "ERROR: LCD AXI base address mismatch. expected=$lcd_axi_baseaddr actual=$actual_lcd_baseaddr"
  exit 1
}

validate_bd_design
save_bd_design
generate_target all [get_files [file join $build_dir "rv32i_lenet_ps_lcd_bd_impl.srcs" "sources_1" "bd" "design_1" "design_1.bd"]]

set wrapper_file [make_wrapper -files [get_files [file join $build_dir "rv32i_lenet_ps_lcd_bd_impl.srcs" "sources_1" "bd" "design_1" "design_1.bd"]] -top]
...
open_run impl_1
report_utilization -file [file join $report_dir "ps_lcd_bd_post_route_util.rpt"]
report_timing_summary -file [file join $report_dir "ps_lcd_bd_post_route_timing.rpt"]
report_drc -file [file join $report_dir "ps_lcd_bd_post_route_drc.rpt"]

set timing_paths [get_timing_paths -max_paths 1 -nworst 1]
set wns [get_property SLACK [lindex $timing_paths 0]]
puts "RV32I_PS_LCD_BD_WNS=$wns"
if {$wns < 0} {
  puts "RV32I_PS_LCD_BD_TIMING_OK=0"
  exit 2
}
puts "RV32I_PS_LCD_BD_TIMING_OK=1"

set bit_file [file join $build_dir "rv32i_lenet_ps_lcd_bd_impl.runs" "impl_1" "design_1_wrapper.bit"]
file copy -force $bit_file [file join $artifact_dir "rv32i_lenet_ps_lcd_bd.bit"]
write_hwdef -force -file [file join $artifact_dir "rv32i_lenet_ps_lcd_bd.hdf"]

puts "RV32I_PS_LCD_BD_IMPL_OK=1"
puts "RV32I_AXI_BASEADDR=$actual_rv32i_baseaddr"
puts "TFTLCD_AXI_BASEADDR=$actual_lcd_baseaddr"
puts "RV32I_PS_LCD_BD_BIT=[file join $artifact_dir rv32i_lenet_ps_lcd_bd.bit]"
puts "RV32I_PS_LCD_BD_HDF=[file join $artifact_dir rv32i_lenet_ps_lcd_bd.hdf]"
