...
set build_dir  [file join $script_dir "build_axi_synth_check"]
set report_dir [file join $script_dir "reports_axi_synth_check"]
...
create_project -force rv32i_lenet_axi_synth_check $build_dir -part $part_name
...
read_verilog [file join $root_dir "Pipeline" "src" "rtl" "RV32I_AxiLite_Top.v"]

synth_design -top RV32I_AxiLite_Top -part $part_name -flatten_hierarchy none
create_clock -name s_axi_aclk -period $clk_period_ns [get_ports s_axi_aclk]
...
puts "RV32I_LENET_AXI_SYNTH_CHECK_WNS=$wns"
if {$wns < 0} {
  puts "RV32I_LENET_AXI_SYNTH_CHECK_TIMING_OK=0"
  exit 2
}
puts "RV32I_LENET_AXI_SYNTH_CHECK_TIMING_OK=1"
