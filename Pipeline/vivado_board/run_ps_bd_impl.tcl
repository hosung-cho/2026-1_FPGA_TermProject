...
set build_dir  [file join $script_dir "build_ps_bd_impl"]
set report_dir [file join $script_dir "reports_ps_bd_impl"]
...
create_project -force rv32i_lenet_ps_bd_impl $build_dir -part $part_name
...
if {[llength $rv32i_seg] == 0} {
  puts "ERROR: RV32I AXI address segment was not created."
  exit 1
}
...
if {$actual_axi_baseaddr ne $expected_axi_baseaddr} {
  puts "ERROR: RV32I AXI base address mismatch. expected=$expected_axi_baseaddr actual=$actual_axi_baseaddr"
  exit 1
}

validate_bd_design
save_bd_design
generate_target all [get_files [file join $build_dir "rv32i_lenet_ps_bd_impl.srcs" "sources_1" "bd" "design_1" "design_1.bd"]]

set wrapper_file [make_wrapper -files [get_files [file join $build_dir "rv32i_lenet_ps_bd_impl.srcs" "sources_1" "bd" "design_1" "design_1.bd"]] -top]
...
open_run impl_1
report_utilization -file [file join $report_dir "ps_bd_post_route_util.rpt"]
report_timing_summary -file [file join $report_dir "ps_bd_post_route_timing.rpt"]
report_drc -file [file join $report_dir "ps_bd_post_route_drc.rpt"]

set timing_paths [get_timing_paths -max_paths 1 -nworst 1]
set wns [get_property SLACK [lindex $timing_paths 0]]
puts "RV32I_PS_BD_WNS=$wns"
if {$wns < 0} {
  puts "RV32I_PS_BD_TIMING_OK=0"
  exit 2
}
puts "RV32I_PS_BD_TIMING_OK=1"

set bit_file [file join $build_dir "rv32i_lenet_ps_bd_impl.runs" "impl_1" "design_1_wrapper.bit"]
file copy -force $bit_file [file join $build_dir "rv32i_lenet_ps_bd.bit"]
write_hwdef -force -file [file join $build_dir "rv32i_lenet_ps_bd.hdf"]

puts "RV32I_PS_BD_IMPL_OK=1"
puts "RV32I_AXI_BASEADDR=$actual_axi_baseaddr"
puts "RV32I_PS_BD_BIT=[file join $build_dir rv32i_lenet_ps_bd.bit]"
puts "RV32I_PS_BD_HDF=[file join $build_dir rv32i_lenet_ps_bd.hdf]"
