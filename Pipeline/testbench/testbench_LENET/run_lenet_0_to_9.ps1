...
wsl.exe -d $WslDistro -- bash -lc "export PATH=$wslPath; cd $testbenchDir; make -f Makefile.verilator TIMEOUT_CYCLES=30000000 all"
...
for ($digit = 0; $digit -lt 10; $digit++) {
    ...
    $cmd = @"
cd $testbenchDir
./obj_dir/VRV32I_System_tb +DMEM_HEX=../../../FPGA_proj/firmware/lenet_digit${digit}_dmem.mem +SAMPLE=digit$digit |
