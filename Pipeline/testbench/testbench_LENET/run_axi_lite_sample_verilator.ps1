...
export PATH=$wslPath
cd $testbenchDir
make -f Makefile.sample_axi_verilator run RUN_ARGS='+IMEM_HEX=../../../FPGA_proj/firmware/lenet_infer_imem.hex +DMEM_HEX=../../../FPGA_proj/firmware/lenet_digit7_dmem.mem' |
  grep '\[SAMPLE_TB\]\( Done\| status=\|\[CHECK_FAIL\]\|\[FAIL\]\|\[PASS\]\)'
"@
