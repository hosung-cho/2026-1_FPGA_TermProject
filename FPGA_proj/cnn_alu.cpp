...
static ap_int<32> requant_relu(ap_int<32> acc, ap_int<32> multiplier) {
#pragma HLS INLINE
    if (acc <= 0) {
        return 0;
    }
    ...
    ap_int<64> rounded = product + ((ap_int<64>)1 << (REQUANT_SHIFT - 1));
    ap_int<32> scaled = (ap_int<32>)(rounded >> REQUANT_SHIFT);
...
static ap_int<32> mac_pack4_delta(ap_uint<32> act_pack, ap_uint<32> weight_pack) {
#pragma HLS INLINE off
#pragma HLS LATENCY min=3 max=3
#pragma HLS ALLOCATION instances=mul limit=2 operation
    ap_int<16> p0 = (ap_int<16>)lane0(act_pack) * (ap_int<16>)lane0(weight_pack);
    ap_int<16> p1 = (ap_int<16>)lane1(act_pack) * (ap_int<16>)lane1(weight_pack);
    ap_int<16> p2 = (ap_int<16>)lane2(act_pack) * (ap_int<16>)lane2(weight_pack);
    ap_int<16> p3 = (ap_int<16>)lane3(act_pack) * (ap_int<16>)lane3(weight_pack);

    ap_int<32> sum01 = (ap_int<32>)p0 + (ap_int<32>)p1;
    ap_int<32> sum23 = (ap_int<32>)p2 + (ap_int<32>)p3;
    return sum01 + sum23;
}
...
void CNN_ALU_Top(
    ...
    #pragma HLS INTERFACE ap_none port=rs2_data
    ...
    switch(cnn_op) {
        ...
        case CMD_MAC_PACK4: {
            acc_reg += mac_pack4_delta(rs1_data, rs2_data);
            break;
        }
        case CMD_POOL4_READ: {
            ap_int<8> a0 = lane0(rs1_data);
            ap_int<8> a1 = lane1(rs1_data);
            ap_int<8> a2 = lane2(rs1_data);
            ap_int<8> a3 = lane3(rs1_data);
            ap_int<8> max1 = (a0 > a1) ? a0 : a1;
            ap_int<8> max2 = (a2 > a3) ? a2 : a3;
            acc_reg = (max1 > max2) ? max1 : max2;
            *rd_data = acc_reg;
            break;
        }
        case CMD_BIAS_REQUANT_RELU_READ: {
            acc_reg = requant_relu(acc_reg + (ap_int<32>)rs1_data, (ap_int<32>)rs2_data);
            *rd_data = acc_reg;
            break;
        }
        case CMD_ADD_BIAS: {
        ...
        case CMD_REQUANT_RELU: {
            // INT8 MAC 결과는 acc_reg에 INT32로 누적됩니다.
            // 다음 layer의 activation buffer는 다시 INT8을 받으므로, layer별 multiplier를
            // 사용해 고정소수점 requantization을 수행합니다.
            //
            //   output_int8 = clamp_relu(round(acc_reg * multiplier / 2^REQUANT_SHIFT))
            //
            // multiplier는 Python export 단계에서
            // input_scale * weight_scale / output_scale을 정수화해서 만듭니다.
            // rs1_data 전체를 signed 32-bit multiplier로 해석합니다.
            acc_reg = requant_relu(acc_reg, (ap_int<32>)rs1_data);
            break;
        }
        default: {
